...
n_opt_aff = 2000 # number of optimization steps
...
Q0_obj = np.eye(N) + np.random.rand(N,N) * 0.5
Q0_obj = 0.5*(Q0_obj+Q0_obj.T)
...
for i_opt in range(n_opt):
    ...
    err_hist[i_opt] = np.linalg.norm(delta_Q0_sim)/np.linalg.norm(Q0_obj)
    Pearson_hist[i_opt] = stt.pearsonr(Q0_obj.flatten(),Q0_sim.flatten())[0]
...
smoothed_err = np.convolve(err_hist,np.ones([n_smooth])/n_smooth,'full')[-n_opt:]
smoothed_Pearson = np.convolve(Pearson_hist,np.ones([n_smooth])/n_smooth,'full')[-n_opt:]

pp.figure(figsize=[2.5,2.5])
pp.axes([0.25,0.2,0.7,0.7])
pp.plot(range(n_opt_aff),smoothed_err[:n_opt_aff],color='k',lw=2)
...
pp.figure(figsize=[2.5,2.5])
pp.axes([0.25,0.2,0.7,0.7])
pp.plot(range(n_opt_aff),smoothed_Pearson[:n_opt_aff],color='k',lw=2)
pp.xticks(np.arange(0,n_opt+1,500),fontsize=10)
pp.yticks([0,0.5,1],fontsize=10)
